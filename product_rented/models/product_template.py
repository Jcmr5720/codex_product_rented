...
class ProductTemplate(models.Model):
    ...
    def _onchange_rented_time_price_ids(self):
        option_ids = self.rented_time_price_ids.mapped("option_id").ids
        self.rented_time_option_ids = [(6, 0, option_ids)]
    ...
    def _compute_synced_option_ids(self, price_commands, existing_lines=None):
        ...
        option_ids = set(existing_lines.mapped("option_id").ids)
        lines_by_id = {line.id: line.option_id.id for line in existing_lines}
        ...
        for command in price_commands or []:
            ...
            if command_type == 0:
                ...
                if "option_id" in data:
                    old_option_id = lines_by_id.get(line_id)
                    if old_option_id:
                        option_ids.discard(old_option_id)
                    new_option_id = data.get("option_id")
                    if new_option_id:
                        option_ids.add(new_option_id)
            elif command_type in (2, 3):
                line_id = command[1]
                option_id = lines_by_id.get(line_id)
                if option_id:
                    option_ids.discard(option_id)
            elif command_type == 5:
                option_ids.clear()
            elif command_type == 6:
                record_ids = command[2] or []
                records = self.env["product.rented.time.price"].browse(record_ids)
                option_ids = set(records.mapped("option_id").ids)
